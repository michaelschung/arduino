...
From this common skeleton, there are three different approaches to programming the binary counter. We will walk through each of them, in order of complexity.
...
```
####Defining Methods
In `void setup()`, set pins `10` through `13` to `OUTPUT` using a simple for-loop. Also begin `Serial` at a baud-rate of 9600 for debug purposes.
```c
void setup() {
	Serial.begin(9600);
	for (int i = 10; i <= 13; i++) {
...
####Defining Variables
At the top of the sketch, define an array to store the pins connected to our LEDs, and another array to store the state which the LED should be in (on or off). `pins` will be an array of type `int` and	`states` will be an array of type `bool` as follows:
```c
...
```	
The algorithm we use needs to continually display the state of the LEDs, change the state, and then display them again. Inside `void loop()`, write the following methods `displayState()`and`changeState()` (as of now, these are unimplemented). Add a 1-second `delay` to temporarily halt execution after each call of loop.
```c
void loop() {
	displayState();
	changeState();
...
```
Our methods `displayState()` and `changesState()` are `void` methods, which means that they do not return a value. They will called repeatedly because they are inside loop.
...
```
##(((((question: why didn't you just do this:
```c
if (states[i] == true) {
	...
	for (int i = 0; i < 4; i++) {
		if (states[i] == true) {
			...
			for (int j = i + 1; j < 4; j++) {
				...
				// if you do change it, make sure to
				// update the explanation after this
				// code block
	...
	for (int i = 0; i < 4; i++) {
		if (states[i] == true) {
			...
			for (int j = i + 1; j < 4; j++) {
				...
				// similar thing here. I think the logic
				...
				// ALSO UPDATE THE FULL CODE DUMP
				// AT THE BOTTOM
